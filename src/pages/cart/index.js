...
function Cart() {
  ...
  const [errorMessage, setErrorMessage] = useState('Your order was canceled 😔')
  ...
  const handleCheckOut = async (method) => {
    ...
    // Compute total price
    const amount = state.reduce((total, food) => total + food.price, 0) // in rupees

    // 💵 CASE 1: Cash on Delivery (COD)
    if (method === 'cod') {
      try {
        const resp = await fetch('/api/payments/createCODOrder', {
          method: 'POST',
        ...
        if (!resp.ok) throw new Error('Failed to place COD order')

        dispatch({ type: 'DROP' })
        setSuccess(true) // show success popup
      } catch (err) {
        console.error(err)
        setErrorMessage('Could not place COD order')
        setFail(true)
      }
      return // Exit function after COD order creation
    }

    // 💳 CASE 2: Online Payment (Razorpay)
    try {
      // Create server-side Razorpay order
      const createResp = await fetch('/api/payments/createOrder', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          amount,
      ...
      if (!createResp.ok) {
        setErrorMessage('Unable to start payment. Try again.')
        setFail(true)
        return
      }

      const { razorpayOrder, paymentId } = createData
      ...
      if (!ok) {
        setErrorMessage('Failed to load payment gateway. Check your internet.')
        setFail(true)
        return
      }
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          const verifyResp = await fetch('/api/payments/verify', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ payment: response, paymentId }),
          })
      ...
      setErrorMessage('Server error while processing payment')
      setFail(true)
    }
  }
